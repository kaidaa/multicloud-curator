...
def _to_response(
    row: repository.LargeStaleCandidateRow | repository.LargeStaleResultRow,
    *,
...
def _evaluation_from_result(result: repository.LargeStaleResultRow) -> LargeStaleEvaluation:
    return LargeStaleEvaluation(
        is_large=result.result.is_large,
        is_stale=result.result.is_stale,
        trigger_reason=result.result.trigger_reason,  # type: ignore[arg-type]
        modified_age_months=result.result.modified_age_months,
    )


def _sort_rows(
    rows: list[tuple[repository.LargeStaleCandidateRow | repository.LargeStaleResultRow, LargeStaleEvaluation]],
    *,
    sort: LargeStaleSort,
) -> list[tuple[repository.LargeStaleCandidateRow | repository.LargeStaleResultRow, LargeStaleEvaluation]]:
    if sort == "size_asc":
...
def list_large_stale_files(
    ...
    *,
...
) -> tuple[list[LargeStaleFileResponse], int, str | None, ScanCoverageResponse | None]:
    del now
    rows = repository.list_large_stale_result_rows(db)
    evaluated: list[tuple[repository.LargeStaleResultRow, LargeStaleEvaluation]] = []
    for row in rows:
        if provider != "all" and row.file.provider != provider:
            continue
        evaluation = _evaluation_from_result(row)
        if category == "large" and not evaluation.is_large:
            continue
        if category == "stale" and not evaluation.is_stale:
            continue
    ...
    scan_at = repository.latest_scan_at(db)
    return (
        [_to_response(row, evaluation=evaluation) for row, evaluation in paginated_rows],
        total,
        _iso(scan_at) if scan_at is not None else None,
        repository.latest_scan_coverage(db),
    )


def run_large_stale_scan(
    db: Session,
    *,
    scanned_at: datetime | None = None,
    now: datetime | None = None,
) -> LargeStaleScanResponse:
    current_scan_at = scanned_at or _utc_now()
    current_time = now or current_scan_at
    account_snapshot = scan_metadata_repo.snapshot_connected_accounts(
        accounts_repo.list_accounts(db)
    )
    rows = repository.list_candidate_files(
        db,
    ...
    results: list[repository.LargeStaleResultInput] = []
    for row in rows:
        evaluation = evaluate_file(row.file, row.account, now=current_time)
        if evaluation.trigger_reason is None:
            continue
        results.append(
            repository.LargeStaleResultInput(
                file_id=row.file.id,
                trigger_reason=evaluation.trigger_reason,
                is_large=evaluation.is_large,
                is_stale=evaluation.is_stale,
                modified_age_months=evaluation.modified_age_months,
            )
        )

    repository.replace_large_stale_results(
        db,
        results=results,
        scanned_at=current_scan_at,
        coverage=coverage,
    )
    return LargeStaleScanResponse(
        scan_at=_iso(current_scan_at),
        total=len(results),
    )
