...
export function QuotaHeroCard({ accounts, problemAccounts }: QuotaHeroCardProps) {
  ...
  return (
    <section className="rounded-[--radius] border border-line bg-panel p-6 shadow-soft">
      ...
      <div className="mt-5 grid gap-6 sm:grid-cols-2">
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-muted-2">Total terpakai</p>
          ...
          <div className="mt-4 h-2 w-full overflow-hidden rounded-full bg-panel-soft">
            <div
              role="progressbar"
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-muted-2">Akun aktif</p>
          <p className="mt-2 text-3xl font-semibold text-ink">
            {activeCount}
            <span className="ml-2 text-sm font-normal text-muted">dari {totalCount}</span>
          </p>
          {problemAccounts.length > 0 ? (
            <div className="mt-4 inline-flex items-center gap-2 rounded-[--radius-sm] bg-warning-soft px-3 py-2 text-xs text-warning-strong">
            ...
            <p className="mt-4 text-xs text-muted">Semua akun sehat.</p>
          )}
        </div>
      </div>
    </section>
